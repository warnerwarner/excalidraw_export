...
async function exportSvg(inputPath) {
  ...
  const outputPathSvg = `${inputPath}.svg`;
  ...
  await page.waitForFunction(() => window.__excalidrawExportToSvg);
  const result = await page.evaluate(async (data) => {
    return await window.renderSvg(data);
  }, json);
